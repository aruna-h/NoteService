...
import com.bridgelabz.discoveryClient_note.model.MetaData;
import com.bridgelabz.discoveryClient_note.model.Note;
import com.bridgelabz.discoveryClient_note.repository.LabelElasticRepository;
import com.bridgelabz.discoveryClient_note.repository.LabelRepository;
import com.bridgelabz.discoveryClient_note.repository.NoteElasticRepository;
import com.bridgelabz.discoveryClient_note.repository.NoteRepository;
import com.bridgelabz.discoveryClient_note.utilservice.PreConditions;
import com.bridgelabz.discoveryClient_note.utilservice.Utility;
import com.bridgelabz.discoveryClient_note.utilservice.messageAccessor.Messages;
import com.bridgelabz.discoveryClient_note.utilservice.redisrepository.RedisRepository;
...
public class NoteService implements INoteService {
	...
	public List<Note> displayAllNotes(String userId) throws ToDoException {
		...
		List<Note> list1 = noteElasticRepository.findNotesByUserId(userId);
		list1.stream()
				.filter(noteStream -> (noteStream.isPinnedStatus() == false
						|| true && noteStream.isTrashStatus() == false && noteStream.isArchivestatus() == false
						|| true))
				.forEach(noteFilter -> newNoteList.add(noteFilter));
		logger.info(userId);
		return newNoteList;
	}
	...
	public void restoreFromTrash(String noteId, String token) throws ToDoException {
		...
		/*
		 * if (!noteElasticRepository.existsById(noteId)) { throw new
		 * ToDoException(messages.get("25")); }
		 */
